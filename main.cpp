...
int MatchDocument(const pair<int, vector<string>>& content, const set<string>& query_words) {
    int res = 0;
    set<string> uniq_content(content.second.begin(), content.second.end());
    for (const string& word : uniq_content) {
        res +=  query_words.count(word);
    }
    return res;
}
...
vector<pair<int, int>> FindDocuments(const vector<pair<int, vector<string>>>& documents,
    ...
    for (const auto& entry : documents) {
        pair<int, int> find_doc;
        int relevance = MatchDocument(entry, query_words);
        if (relevance) {
            find_doc.first = entry.first;
            find_doc.second = relevance;
            matched_documents.push_back(find_doc);
        }
    }
